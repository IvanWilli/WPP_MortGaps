...
country_i = "United States of America"; dates_out = 1950.5:2020.5;
...
# strategy
# get complete data lx or mx
single_data <- country_data %>% filter(IndicatorName %in% c("m(x,n) - complete",
                                                            "l(x) - complete"),
                                        SexName!="Both sexes",
                                        DataSourceShortName %in% c("HMD","EuroStat","HLD 2020"))

plot_age_time(single_data %>% rename(Age=AgeStart,Date=TimeMid, Sex=SexName, nMx=DataValue) %>% 
                      filter(DataSourceShortName=="HMD",
                             IndicatorName %in% c("m(x,n) - complete")))

# detect gaps for single data
