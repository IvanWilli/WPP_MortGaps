...
fill_gaps_tier1 <- function(country = NULL, dates_out = 1950.5:2020.5,
                           LC_fit_e0 = FALSE, window_LC = 30, window_LClim = 5,
                           exclude_ds = NULL, OAnew = 100,
                           prev_cross_over = FALSE, jump_off = TRUE){

        cat(country)
        
        country = "Belgium"; dates_out = 1950.5:2020.5;
        LC_fit_e0 = FALSE; window_LC = 30; window_LClim = 5; exclude_ds = NULL;
        OAnew = 100; prev_cross_over = FALSE; jump_off = FALSE
        ...
        # data for model tables uses - not used in tier1 (?)
        ...
        if(length(intervals_gaps[[1]])!=0){
                ...
                for(gap in intervals_gaps){
                        ...
                        if(1950.5 %in% gap){
                                if(any(unique(lt_data_no_zeros$Date) %in% gap)){ # here goes model lt data
                                        ...
                                        if(!LC_fit_e0){
                                                ...
                                                dates_e0 <- lt_data_no_zeros %>% filter(Type!="main", Age==0) %>% select(Date) %>% unique() %>% pull()
                                                e0_Males <- lt_data_no_zeros %>% filter(Type!="main", Age==0, Sex=="m") %>% select(ex) %>% pull()
                                                e0_Females <- lt_data_no_zeros %>% filter(Type!="main", Age==0, Sex=="f") %>% select(ex) %>% pull()
                                                gap_before_data <- interp_lc_lim(input = LClim_data, dates_out = as.numeric(gap), 
        ...
        # plots
        g_country_rates <- plot_age_time(t_final_data)
        g_country_ex <- plot_ex_time(t_final_data)
        g_dispersion <- plot_dispersion(t_dx_disperison)
        g_sex_ratios <- plot_sex_ratios(t_sex_ratio)
